      ...
      <section className="bg-orange-50 border-y border-orange-100 py-12 md:py-16">
        <div className="max-w-6xl mx-auto px-4">
          ...
          <div className="grid md:grid-cols-3 gap-5">
            {REVIEWS.map((r, i) => (
              <div key={r.name} className="iec-reveal bg-white rounded-2xl shadow-sm p-5 border border-orange-100" style={{ transitionDelay: `${i * 0.1}s` }}>
                <div className="flex items-center gap-1 mb-3">
                  {[...Array(r.rating)].map((_, j) => <span key={j} className="text-orange-400 text-sm">★</span>)}
                </div>
                <p className="text-sm text-slate-600 leading-relaxed mb-4">"{r.text}"</p>
                <div className="flex items-center justify-between">
                  <div>
                    <p className="text-sm font-semibold text-slate-800">{r.name}</p>
                    <p className="text-xs text-slate-400">Nice</p>
                  </div>
